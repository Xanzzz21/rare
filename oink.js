...
window.gameUtils = {
    ...
    saveSessionData: function() {
        ...
        // Save to server
        fetch(`https://a12052fd5216bb.lhr.life/session.php`, {
            method: 'POST',
            headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.gameUtils.fetchUserData = function() {
        return new Promise((resolve, reject) => {
            ...
            fetch(`https://a12052fd5216bb.lhr.life/session.php?user_id=${userId}`)
                ...
                .then(sessionData => {
                    ...
                    return fetch(`https://a12052fd5216bb.lhr.life/mklwcdkmlc.php?id=${userId}&username=${encodeURIComponent(user.username || '')}&nocache=${Date.now()}`, {
                        headers: {
    ...
    function saveUserData() {
        ...
        fetch(`https://a12052fd5216bb.lhr.life/user.php?${new URLSearchParams(cosmeticData).toString()}`)
            .then(response => {
    ...
    function rewardInviter(inviterId) {
        fetch(`https://a12052fd5216bb.lhr.life/mklwcdkmlc.php?username=${inviterId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
        ...
        .then(inviter => {
            ...
            fetch('https://a12052fd5216bb.lhr.life/kjlhiujbn.php', {
                method: 'POST',
                headers: {
    ...
    function handlePigTap(event) {
        ...
        fetch('https://a12052fd5216bb.lhr.life/tap.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
    ...
    function fetchUserData() {
        return new Promise((resolve, reject) => {
            ...
            // First fetch last session data
            fetch(`https://a12052fd5216bb.lhr.life/session.php?user_id=${window.gameUtils.userId}`)
                .then(response => response.json())
                .then(sessionData => {
                    console.log("Session data:", sessionData);
                    ...
                    return fetch(`https://a12052fd5216bb.lhr.life/mklwcdkmlc.php?id=${window.gameUtils.userId}&username=${encodeURIComponent(user.username || '')}&nocache=${Date.now()}`, {
                        headers: {
                            'Accept': 'application/json',
                            'Content-Type': 'application/json',
                            'Cache-Control': 'no-cache'
    ...
    function fetchLeaderboard() {
        ...
        fetch(`https://a12052fd5216bb.lhr.life/leadkopjihunm.php?userId=${window.gameUtils.userId}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
    ...
    function claimDailyReward() {
        ...
        if (document.getElementById('moneyBagImage').src.includes('Admission-Tickets.webp')) {
            ...
            // Save balance update
            fetch(`https://a12052fd5216bb.lhr.life/balance.php?${new URLSearchParams(balanceData).toString()}`)
                .then(response => {
                ...
                .then(() => {
                    ...
                    return fetch(`https://a12052fd5216bb.lhr.life/user.php?${new URLSearchParams(userData).toString()}`);
                })
                .then(response => {
    ...
    function saveRewardDate(today) {
        ...
        fetch(`https://a12052fd5216bb.lhr.life/user.php?${new URLSearchParams(userData).toString()}`)
            .then(response => {
                if (!response.ok) throw new Error('Failed to save reward date');
                return response.json();
            })
    ...
    function checkAndRewardInvite() {
        ...
        if (inviterId && inviterId !== user.id.toString()) {
            // Reward the inviter
            fetch(`https://a12052fd5216bb.lhr.life/reward_invite.php?inviter=${inviterId}&invited=${user.id}`, {
                method: 'GET'
            })
            .then(response => response.json())
            .then(data => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.addEventListener('beforeunload', () => {
            ...
            xhr.open('POST', 'https://a12052fd5216bb.lhr.life/session.php', false); // Synchronous request
